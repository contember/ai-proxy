...
// showPasswordDialog shows a native macOS password input dialog (hidden text)
func showPasswordDialog(title, prompt string) (string, bool, error) {
	script := fmt.Sprintf(`
		set dialogResult to display dialog %q default answer "" with hidden answer with title %q buttons {"Cancel", "OK"} default button "OK"
		if button returned of dialogResult is "OK" then
			return text returned of dialogResult
		else
			return ""
